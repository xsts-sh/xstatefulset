...
var patchCodec = legacyscheme.Codecs.LegacyCodec(xstsappv1.SchemeGroupVersion)
...
func getStartOrdinal(set *xstsappv1.XStatefulSet) int {
...
func getEndOrdinal(set *xstsappv1.XStatefulSet) int {
...
func podInOrdinalRange(pod *v1.Pod, set *xstsappv1.XStatefulSet) bool {
...
func getPodName(set *xstsappv1.XStatefulSet, ordinal int) string {
...
func getPersistentVolumeClaimName(set *xstsappv1.XStatefulSet, claim *v1.PersistentVolumeClaim, ordinal int) string {
	// NOTE: This name format is used by the heuristics for zone spreading in ChooseZoneForVolume
	return fmt.Sprintf("%s-%s-%d", claim.Name, set.Name, ordinal)
}

// isMemberOf tests if pod is a member of set.
func isMemberOf(set *xstsappv1.XStatefulSet, pod *v1.Pod) bool {
	return getParentName(pod) == set.Name
}

// identityMatches returns true if pod has a valid identity and network identity for a member of set.
func identityMatches(set *xstsappv1.XStatefulSet, pod *v1.Pod) bool {
	parent, ordinal := getParentNameAndOrdinal(pod)
	return ordinal >= 0 &&
		set.Name == parent &&
		pod.Name == getPodName(set, ordinal) &&
		pod.Namespace == set.Namespace &&
		pod.Labels[xstsappv1.StatefulSetPodNameLabel] == pod.Name
}

// storageMatches returns true if pod's Volumes cover the set of PersistentVolumeClaims
func storageMatches(set *xstsappv1.XStatefulSet, pod *v1.Pod) bool {
	ordinal := getOrdinal(pod)
	if ordinal < 0 {
		return false
	}
...
func getPersistentVolumeClaimRetentionPolicy(set *xstsappv1.XStatefulSet) apps.StatefulSetPersistentVolumeClaimRetentionPolicy {
...
func hasUnexpectedController(claim *v1.PersistentVolumeClaim, set *xstsappv1.XStatefulSet, pod *v1.Pod) bool {
...
func hasNonControllerOwner(claim *v1.PersistentVolumeClaim, set *xstsappv1.XStatefulSet, pod *v1.Pod) bool {
	for _, ownerRef := range claim.GetOwnerReferences() {
...
func isClaimOwnerUpToDate(logger klog.Logger, claim *v1.PersistentVolumeClaim, set *xstsappv1.XStatefulSet, pod *v1.Pod) bool {
...
func updateClaimOwnerRefForSetAndPod(logger klog.Logger, claim *v1.PersistentVolumeClaim, set *xstsappv1.XStatefulSet, pod *v1.Pod) {
...
func getPersistentVolumeClaims(set *xstsappv1.XStatefulSet, pod *v1.Pod) map[string]v1.PersistentVolumeClaim {
	ordinal := getOrdinal(pod)
...
func updateStorage(set *xstsappv1.XStatefulSet, pod *v1.Pod) {
...
func initIdentity(set *xstsappv1.XStatefulSet, pod *v1.Pod) {
...
func updateIdentity(set *xstsappv1.XStatefulSet, pod *v1.Pod) {
	ordinal := getOrdinal(pod)
	pod.Name = getPodName(set, ordinal)
	pod.Namespace = set.Namespace
	if pod.Labels == nil {
	...
	pod.Labels[xstsappv1.StatefulSetPodNameLabel] = pod.Name
	pod.Labels[xstsappv1.PodIndexLabel] = strconv.Itoa(ordinal)
}
...
func allowsBurst(set *xstsappv1.XStatefulSet) bool {
...
func setPodRevision(pod *v1.Pod, revision string) {
	...
	pod.Labels[xstsappv1.StatefulSetRevisionLabel] = revision
}
...
func getPodRevision(pod *v1.Pod) string {
	...
	return pod.Labels[xstsappv1.StatefulSetRevisionLabel]
}

// newStatefulSetPod returns a new Pod conforming to the set's Spec with an identity generated from ordinal.
func newStatefulSetPod(set *xstsappv1.XStatefulSet, ordinal int) *v1.Pod {
...
func newVersionedStatefulSetPod(currentSet, updateSet *xstsappv1.XStatefulSet, currentRevision, updateRevision string, ordinal int) *v1.Pod {
...
// recorded patches.
func getPatch(set *xstsappv1.XStatefulSet) ([]byte, error) {
...
func newRevision(set *xstsappv1.XStatefulSet, revision int64, collisionCount *int32) (*apps.ControllerRevision, error) {
...
func ApplyRevision(set *xstsappv1.XStatefulSet, revision *apps.ControllerRevision) (*xstsappv1.XStatefulSet, error) {
	clone := set.DeepCopy()
	patched, err := strategicpatch.StrategicMergePatch([]byte(runtime.EncodeOrDie(patchCodec, clone)), revision.Data.Raw, clone)
	if err != nil {
		return nil, err
	}
	restoredSet := &xstsappv1.XStatefulSet{}
...
func inconsistentStatus(set *xstsappv1.XStatefulSet, status *xstsappv1.XStatefulSetStatus) bool {
...
// are set to 0.
func completeRollingUpdate(set *xstsappv1.XStatefulSet, status *xstsappv1.XStatefulSetStatus) {
