...
import (
	...
	kthenaclientset "github.com/xsts-sh/xstatefulset/client-go/clientset/versioned"
	appsv1informers "github.com/xsts-sh/xstatefulset/client-go/informers/externalversions/apps/v1"
	appslisters "github.com/xsts-sh/xstatefulset/client-go/listers/apps/v1"
	"github.com/xsts-sh/xstatefulset/pkg/controller"
	"github.com/xsts-sh/xstatefulset/pkg/controller/history"
	"github.com/xsts-sh/xstatefulset/pkg/controller/legacyscheme"
	podutil "github.com/xsts-sh/xstatefulset/pkg/controller/utils"
	"github.com/xsts-sh/xstatefulset/pkg/controller/xstatefulset/metrics"
...
var controllerKind = xstsappv1.SchemeGroupVersion.WithKind("XStatefulSet")
...
type StatefulSetController struct {
	...
	kthenaClientset kthenaclientset.Interface
...
// NewStatefulSetController creates a new xstatefulset controller.
func NewStatefulSetController(
	ctx context.Context,
	podInformer coreinformers.PodInformer,
	localSetInformer appsv1informers.XStatefulSetInformer,
	...
	recorder := eventBroadcaster.NewRecorder(legacyscheme.Scheme, v1.EventSource{Component: "xstatefulset"})

	// Register metrics
	metrics.Register()
	ssc := &StatefulSetController{
		kubeClient:      kubeClient,
		kthenaClientset: kthenaClientSet,
		control: NewDefaultStatefulSetControl(
			NewStatefulPodControl(
				...
				pvcInformer.Lister(),
				recorder),
			NewRealStatefulSetStatusUpdater(kthenaClientSet, localSetInformer.Lister()),
		...
		queue: workqueue.NewTypedRateLimitingQueueWithConfig(
			...
			workqueue.TypedRateLimitingQueueConfig[string]{Name: "xstatefulset"},
		),
	...
	podInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		// lookup the xstatefulset and enqueue
		AddFunc: func(obj interface{}) {
			ssc.addPod(logger, obj)
		},
		// lookup current and old xstatefulset if labels changed
		UpdateFunc: func(oldObj, newObj interface{}) {
			ssc.updatePod(logger, oldObj, newObj)
		},
		// lookup xstatefulset accounting for deletion tombstones
	...
	localSetInformer.Informer().AddEventHandler(
		cache.ResourceEventHandlerFuncs{
			AddFunc: func(obj interface{}) {
				ssc.enqueueStatefulSet(logger, obj)
			},
			UpdateFunc: func(old, cur interface{}) {
				oldPS := old.(*xstsappv1.XStatefulSet)
				curPS := cur.(*xstsappv1.XStatefulSet)
	...
	ssc.setLister = localSetInformer.Lister()
	ssc.setListerSynced = localSetInformer.Informer().HasSynced

	// TODO: Watch volumes
	return ssc
}

// Run runs the xstatefulset controller.
func (ssc *StatefulSetController) Run(ctx context.Context, workers int) {
	...
	defer func() {
		logger.Info("Shutting down xstatefulset controller")
		ssc.queue.ShutDown()
		wg.Wait()
	}()
...
// addPod adds the xstatefulset for the pod to the sync queue
...
// updatePod adds the xstatefulset for the current and old pods to the sync queue.
...
// deletePod enqueues the xstatefulset for the pod accounting for deletion tombstones.
...
func (ssc *StatefulSetController) getPodsForStatefulSet(ctx context.Context, set *xstsappv1.XStatefulSet, selector labels.Selector) ([]*v1.Pod, error) {
	podsForSts, err := controller.FilterPodsByOwner(ssc.podIndexer, &set.ObjectMeta, controllerKind.Kind, true)
	if err != nil {
		return nil, err
	}
...
func (ssc *StatefulSetController) canAdoptFunc(ctx context.Context, set *xstsappv1.XStatefulSet) func(ctx2 context.Context) error {
	return controller.RecheckDeletionTimestamp(func(ctx context.Context) (metav1.Object, error) {
		fresh, err := ssc.kthenaClientset.AppsV1().XStatefulSets(set.Namespace).Get(ctx, set.Name, metav1.GetOptions{})
		if err != nil {
			return nil, err
		}
...
func (ssc *StatefulSetController) adoptOrphanRevisions(ctx context.Context, set *xstsappv1.XStatefulSet) error {
...
// a given pod.
func (ssc *StatefulSetController) getStatefulSetsForPod(logger klog.Logger, pod *v1.Pod) []*xstsappv1.XStatefulSet {
...
// of the correct Kind.
func (ssc *StatefulSetController) resolveControllerRef(namespace string, controllerRef *metav1.OwnerReference) *xstsappv1.XStatefulSet {
	// We can't look up by UID, so look up by Name and then verify UID.
	// Don't even try to look up by Name if it's the wrong Kind.
	if controllerRef.Kind != controllerKind.Kind {
		return nil
	}
	set, err := ssc.setLister.XStatefulSets(namespace).Get(controllerRef.Name)
	if err != nil {
		return nil
	}
...
// enqueueStatefulSet enqueues the given xstatefulset in the work queue.
...
// enqueueStatefulSet enqueues the given xstatefulset in the work queue after given time
func (ssc *StatefulSetController) enqueueSSAfter(logger klog.Logger, ss *xstsappv1.XStatefulSet, duration time.Duration) {
...
// sync syncs the given xstatefulset.
func (ssc *StatefulSetController) sync(ctx context.Context, key string) error {
	startTime := time.Now()
	logger := klog.FromContext(ctx)
	defer func() {
		logger.V(4).Info("Finished syncing xstatefulset", "key", key, "time", time.Since(startTime))
	}()
	...
	set, err := ssc.setLister.XStatefulSets(namespace).Get(name)
	...
	// Apply defaults to the XStatefulSet object
	// This ensures that fields like RevisionHistoryLimit have their default values
	// even if they weren't set when the object was created
...
// syncStatefulSet syncs a tuple of (xstatefulset, []*v1.Pod).
func (ssc *StatefulSetController) syncStatefulSet(ctx context.Context, set *xstsappv1.XStatefulSet, pods []*v1.Pod) error {
	logger := klog.FromContext(ctx)
	logger.V(4).Info("Syncing StatefulSet with pods", "statefulSet", klog.KObj(set), "pods", len(pods))
	var status *xstsappv1.XStatefulSetStatus
	var err error
